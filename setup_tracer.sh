...
# 2) Torch & core wheels  (always inside venv)
TORCH_VER=2.4.1
...
# 4) patch dataclass mutable defaults (Python 3.12)
python - <<'PY'
...
# 5) write a permanent stub for torchtext.vocab.vocab
python - <<'PY'
...
pth.write_text(textwrap.dedent(stub_code).strip() + "\\n")
PY
